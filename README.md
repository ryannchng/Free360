...
For the managed Supabase option, each cloned deployment is configured for **one Supabase project and one circle**. The person setting up the group does these steps once:
...
Sharing is off until a member enables it. Foreground updates and check-ins upload through HTTPS; the app checks for updates while open (every five seconds with the standalone server). The latest encrypted snapshot is queued on the device during an outage and retried after reconnect. Intermediate points are not retained. A stale map marker is not a safety confirmation.
